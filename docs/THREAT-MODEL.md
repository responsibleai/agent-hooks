...
| TM-09 | Audit-record payload leakage: records or failure messages exfiltrate context data into audit storage | I | In | Identity-only record envelope; failure verdicts carry exception *type* only (NOW-05, incl. Go panic recovery) | Record shape: `spec/schema/interception-record.schema.json` + `sdk/python/tests/test_decided_by.py`; Go panic tests assert the payload text does not leak. Systematic leak test across all failure paths: **GAP**. Note: the record's combined verdict can carry `transform.value` and free-form messages — see the open payload-free-record decision (arch-review 2026-07-09 NOW-07) |
...
Every **GAP** above, with the tracking item (roadmap ids reference the
2026-07-09 arch-review unless noted):

| Gap | Threat rows | Tracked by |
| --- | --- | --- |
| No adversarial *mutate* fault in the vector grammar (interceptor mutates its copy in place) | TM-05 | NEXT (fault-grammar extension) |
| No systematic test that every failure path keeps verdict messages payload-free | TM-09 | follow-up to NOW-05 |
| Record's combined verdict embeds `transform.value`/messages — "payload-free" claim scope | TM-09 | NOW-07 (decision pending) |
| `verification_pointers` no-dereference is prose-only; no test or scheme guidance | TM-10 | NEXT (SSRF guidance) |
...
| §5.4 `result_labels` persistence/`source_labels` resurfacing across emissions untested | TM-13 | NEXT (§5.4 vector) |
| Lockfile enforcement, dependency automation, SBOM/signing/provenance | TM-14 | NOW-10/NOW-11 (in flight) |
