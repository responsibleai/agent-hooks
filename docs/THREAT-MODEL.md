...
| §12.1 streaming fail-closed path untested on the buffered surface (no partial-stream form for a buffering host) | TM-11 | narrowed — the incremental surface is closed by `streaming/incremental` (`AH-CTK-112` pins the `host_error:streaming_unsupported` residue path); the buffered negative path stays prose + per-SDK unit tests, and the §12.1a egress rule itself is decided and normative |
